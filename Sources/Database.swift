...
class DB {
	...
	func getList() -> [[String: String]] {
		...
		do {
			...
			let demoStatement = "SELECT id,name,body,time FROM blogData"

			try sqlite.forEachRow(statement: demoStatement, handleRow: {(statement: SQLiteStmt, i:Int) -> () in
				var contentDict = [String: String]()
				contentDict["id"] = String(statement.columnText(position: 0))
				contentDict["name"] = String(statement.columnText(position: 1))
				contentDict["body"] = String(statement.columnText(position: 2))
				contentDict["time"] = String(statement.columnText(position: 3))
	...
	func addData(postData: String) {
		...
		do {
			...
			print("+++++++++")
			print(postData)
			// let title = postData.NSRegularExpression(pattern: "^title=.*&body=.*$")
			// _ = postData.pregMatche(pattern: "^title=.*&body=", matches: &titleRegex)   // 返り値を使わないとエラーが出る。ワイルドカードで対処
			_ = postData.pregMatche(pattern: "^body=.*&name=", matches: &bodyRegex)   // 返り値を使わないとエラーが出る。ワイルドカードで対処
			_ = postData.pregMatche(pattern: "&name=.*$", matches: &nameRegex)   // 返り値を使わないとエラーが出る。ワイルドカードで対処
			// var title = titleRegex[0].substring(to: titleRegex[0].index(titleRegex[0].endIndex, offsetBy: -6))
			// title = title.substring(from: title.index(title.startIndex, offsetBy: 6))
			var body = bodyRegex[0].substring(to: bodyRegex[0].index(bodyRegex[0].endIndex, offsetBy: -6))
			body = body.substring(from: body.index(body.startIndex, offsetBy: 5))
			var name = nameRegex[0].substring(to: nameRegex[0].index(nameRegex[0].endIndex, offsetBy: 0))
			name = name.substring(from: name.index(name.startIndex, offsetBy: 6))
			print(body)
			print(name)
			try sqlite.execute(statement: "INSERT INTO blogData(name, body) VALUES(?, ?)", doBindings: {
				(stmt: SQLiteStmt) -> () in
				try stmt.bind(position: 1, name)
				try stmt.bind(position: 2, body)
			})
			try sqlite.execute(statement: "INSERT INTO blogData(name, body) VALUES('otyazuke', ?)", doBindings: {
				(stmt: SQLiteStmt) -> () in
				try stmt.bind(position: 1, body)
			})
			print("+++++++++")
